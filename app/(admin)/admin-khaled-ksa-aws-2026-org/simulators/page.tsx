...
import {
  ...
  IconBrush, IconAlertTriangle, IconSettings,
...
export default function AdminCoursesPage() {
  ...
  useEffect(() => {
    setIsMounted(true);
    fetchCourses('simulator').then(data => {
  ...
  return (
    <div className="flex flex-col gap-6 pb-10" dir="rtl">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-text">إدارة المحاكيات</h1>
          <p className="text-sm text-text-muted mt-1">أضف وعدّل المحاكيات التفاعلية الخاصة باختبار ستيب والقدرات</p>
        </div>
        <button
          onClick={openAdd}
        ...
        >
          <IconPlus size={18} /> إضافة محاكي جديد
        </button>
      </div>
      ...
      {courses.length === 0 && (
        <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border-2 border-dashed border-border bg-card py-20 text-center">
          ...
          <p className="font-bold text-text-muted">لا توجد محاكيات بعد</p>
          ...
          <div key={course.id} className={`rounded-2xl border bg-card overflow-hidden ${course.isActive ? "border-border" : "border-border opacity-60"}`}>
            ...
            <div className="p-4 flex flex-col gap-3">
              ...
              <div className="flex items-center justify-between text-xs text-text-muted border-t border-border pt-3">
                <span className="flex items-center gap-1"><IconClock size={13} /> {course.totalHours ? `${course.totalHours} دقيقة` : "لم تُحدد المدة"}</span>
              ...
              <div className="flex items-center gap-2 pt-1">
                ...
                <Link
                  href={`/admin-khaled-ksa-aws-2026-org/simulators/${course.id}`}
                  className="rounded-lg p-2 text-indigo-500 hover:bg-indigo-500/10 font-bold text-xs flex items-center gap-1 transition-colors"
                >
                  <IconSettings size={15} /> إدارة الأسئلة
                </Link>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/60 p-4 pt-8">
          <div className="w-full max-w-2xl rounded-2xl bg-card border border-border shadow-2xl">
            ...
            <div className="flex items-center justify-between border-b border-border px-6 py-4">
              <h2 className="text-lg font-black">{editId ? "تعديل المحاكي" : "إضافة محاكي جديد"}</h2>
            ...
            <div className="flex flex-col gap-5 p-6">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-text-muted mb-1.5">عنوان المحاكي *</label>
                  <input
                    value={form.title}
                    onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
                    placeholder="مثال: محاكي القدرات الشامل 2026"
                ...
                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-text-muted mb-1.5">وصف المحاكي</label>
                  <textarea
                    value={form.description}
                    onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                    rows={3}
                    placeholder="اكتب وصفاً شاملاً للمحاكي..."
                ...
                <div className="sm:col-span-2">
                  <label className="block text-xs font-bold text-text-muted mb-1.5 flex items-center gap-1"><IconClock size={13} /> مدة الاختبار بالدقائق *</label>
                  <input
                    type="number"
                    min={1}
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm font-bold">تفعيل المحاكي</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <div
                  ...
                  <span className="text-sm font-bold">تمييزه في الأعلى</span>
                </label>
              </div>
            </div>
            ...
            <div className="flex gap-3 border-t border-border px-6 py-4">
              ...
              >
                {editId ? "حفظ التعديلات" : "إضافة المحاكي"}
              </button>
